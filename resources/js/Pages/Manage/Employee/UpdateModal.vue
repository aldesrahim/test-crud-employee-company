...
const openModal = async (employeeId) => {
    const url = route('manage.employee.show', {employee: employeeId});
    window.axios.get(url)
        ...
        .catch(error => {
            notification['error']({
                message: `Unable to retrieve employee ${employeeData.value?.name} data`,
                placement: 'topRight',
            })
...
const fillForm = () => {
    if (!employeeData.value?.id) {
        notification['error']({
            message: `Employee with ID: ${employeeData.value?.id} could not be found`,
            placement: 'topRight',
        })
    ...
    open.value = true
    form.company_id = employeeData.value.company_id
    form.first_name = employeeData.value.first_name
    form.last_name = employeeData.value.last_name
    form.email = employeeData.value.email
    form.phone = employeeData.value.phone
}
...
const onSubmit = () => {
    ...
    form
        .put(route('manage.employee.update', {id: employeeData.value?.id}), {
            onError: (params) => {
                notification['error']({
